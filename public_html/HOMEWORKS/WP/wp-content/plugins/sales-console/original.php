    ...
    <tr>
        <td align="left" valign="top">
            <form method="POST" action="" name="cart">
                <table id="formtable" width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                        ...
                        <td width="93%"><input type="text" id="ID" name="product_id"></td>
                    </tr>
                    <tr>
                ...
                <input type="submit" name="button" class="button-primary" value="Add item">
            </form>
            <?php
            if($_REQUEST['product_id'] or $_REQUEST['ISBN'] or $_REQUEST['credit']) :
                global $wpdb;
                $remove_id = $_REQUEST['product_id'];
                $barcode_id = $_REQUEST['product_id'];
                $ISBN = $_REQUEST['ISBN'];
                $qty = $_REQUEST['quantity'];
                $action = $_REQUEST['action'];
                if($_REQUEST['product_id']) {
                    $BARCODEstr = "SELECT post_id
						FROM $wpdb->postmeta
						WHERE (meta_key = '_cmb_resource_barcode' AND meta_value = '$barcode_id')
						GROUP BY post_id;
					";
                    $BARCODE_id = explode(",",$wpdb->get_var($wpdb->prepare($BARCODEstr)));
                    $product_id = $BARCODE_id[0];
                } else if($_REQUEST['ISBN']) {
                    $ISBNstr = "SELECT post_id
						FROM $wpdb->postmeta
						WHERE (meta_key = '_cmb_resource_sku' AND meta_value = '$ISBN') 
						OR (meta_key = '_cmb_resource_u-sku' AND meta_value = '$ISBN')
						GROUP BY post_id;
					";
                    $ISBN_id = explode(",",$wpdb->get_var($wpdb->prepare($ISBNstr)));
                    $product_id = $ISBN_id[0];
                } else { };
                switch($action) {
                    case "add":
                        ...
                        break;
                    case "credit":
                        $_SESSION['credit'] = $_REQUEST['credit'];
                        break;
                    case "remove":
                        $_SESSION['cart'][$remove_id]--;
                        if($_SESSION['cart'][$remove_id] == 0)
            ...
            <?php if($_SESSION['cart']) : ?>
                <p>&nbsp;</p>
                <table id="bookbag" width="100%" border="0" cellspacing="0" cellpadding="0">
                    <thead>
                    <th align="left" width="55%">Item</th>
                    <th width="5%">Quantity</th>
                    <th width="10%">&nbsp;</th>
                    <th align="left" width="10%" class="price">Price</th>
                    <th width="20%">&nbsp;</th>
                    </thead>
                    <?php foreach($_SESSION['cart'] as $product => $qty) :
                        ...
                        <tr>
                            ...
                            <td>&nbsp;</td>
                            <td class="price">$<?php echo number_format($line_cost, 2); ?></td>
                            <td>&nbsp;</td>
                        </tr>
                    <?php endforeach; ?>
                    <tr>
                    ...
                    <tr class="calculate">
                        ...
                        <?php if ($credit=="0.00" or $credit=="0" or $credit==" ") { ?>
                            <td>$0.00</td>
                            <td>&nbsp;</td>
                        <?php } else { ?>
                            <td>$<?php echo number_format($credit, 2); ?></td>
                            <td>&larr; Discount applied</td>
                        <?php } ?>
                    </tr>
                    <tr class="calculate">
                        <td colspan="2">&nbsp;</td>
                        ...
                        <td>$<?php echo $tax; ?></td>
                        <?php if ($tax=="0.00") { ?>
                            <td>&larr; Tax rate not set</td>
                        <?php } else { ?>
                            <td>&larr; <?php echo get_option('ctaxrate'); ?>% applied</td>
                        <?php } ?>
                    </tr>
                    <tr class="calculate">
                        <td colspan="2">&nbsp;</td>
                    ...
                    <tr class="calculate">
                        ...
                        <td>
                            <form id="empty" action="" method="post">
                                ...
                                <input type="submit" class="button-primary" value="Empty bookbag" />
                            </form>
                        </td>
                    </tr>
                </table>
        ...
        <td id="payment-column" align="left" valign="top">
            ...
            <?php }; ?>
                <div style="display: none;">
                    <div id="toPrint">
                        ...
                        <p style="text-align: center;"><?php echo get_option('invoiceaddress'); ?><br /><strong>Phone:</strong> <?php echo get_option('invoicephone'); ?><br />Come visit us online at <?php echo get_option('invoiceURL'); ?></p>
                        <p>&nbsp;</p>
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                        ...
                        <td align="left" valign="top">
                            <form id="clear" action="<?php echo admin_url('admin.php?page=conference-sales','https'); ?>" method="post">
                                <input type="hidden" name="product_id" value="null" />
                                <input type="hidden" name="action" value="empty" />
                        ...
                        <td valign="top">
                            <?php if($_REQUEST['cn']) :
                                ...
                                <p><strong>Name:</strong> <?php echo $_REQUEST['ch']; ?><br />
                                    ...
                                    <?php echo $_REQUEST['AVSCITY']; ?><?php if($_REQUEST['AVSSTATE'] !=""){ ?>, <?php }; ?><?php echo $_REQUEST['AVSSTATE']; ?> <?php echo $_REQUEST['AVSZIP']; ?></p>
