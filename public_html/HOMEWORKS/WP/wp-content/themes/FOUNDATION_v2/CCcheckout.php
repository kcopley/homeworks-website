		...
		<?php if($_SESSION['cart']) : ?>    
			    ...
			    <th scope="col" id="listprice">List price</th>
					...
					foreach($_SESSION['cart'] as $product => $qty) : ?>
						...
						if ($type == 'classes') {
							...
							$ctotal = $ctotal + $cline_cost;
						} else {
							$MSRP = Book::$props[Book::$msrp]->GetValue($product);
							$pprice = Book::$props[Book::$price]->GetValue($product);
							$pamount = str_replace("$","",$pprice);
							$pline_cost = $pamount * $qty;
							$ptotal = $ptotal + $pline_cost;
						}; ?>
						...
						$taxrate = (float) get_option('taxrate') / 100;
						$tax = $taxrate*$total;
						$shipping = (float) get_option('flatrate');
						$cost = number_format($total + $shipping,2);
						...
						<tr>
							...
							<td>
								...
								<?php if ($type == 'classes') { 
									...
									<strike><?php echo $MSRP * $qty; ?></strike>
			...
			<form id="TRANSFIRST" method="POST" action="<?php bloginfo('url'); ?>/order" name="frmReturn" id="frmReturn">
				...
				?>
                           	...
                           	<input type="hidden" name="RefID" value="<?php echo 0; ?>">
