...
class Http
{
      ...
      if( !is_array($this->headerArray) )
	{
      ...
      $key = trim(explode(':',$str,1));
      $f = 0;
      for( $i = 0; $i < count($this->headerArray); $i++ )
	{
	  $key2 = trim(explode(':',$this->headerArray[$i],1));
	  if( $key2 == $key1 )
	    {
	      // found a duplicate.
	...
	$cookieString = '';
        ...
        else
        {
            ...
            $this->addRequestHeader($this->method . ": " . $this->path . "  HTTP/1.1");
            $this->addRequestHeader("Host: " . $urlParsed['host']);
