...
function cge_CreateInputYesNoDropdown(&$mod,$id,$name,$selectedvalue='',$addtext='')
  {
    ...
    $items = array($cgextensions->Lang('yes')=>1,
		   $cgextensions->Lang('no')=>0);
    return $mod->CreateInputDropdown($id,$name,$items,-1,$selectedvalue,$addtext);
  }
  

function cge_CGCreateInputSubmit(&$mod,$id,$name,$value='',$addtext='',$image='',
			     $confirmtext='',$class='')
  {
    $real_image = '';
    if( !empty($image) )
	...
	if( isset($mod->_image_directories) && 
	    !empty($mod->_image_directories))
	  {
