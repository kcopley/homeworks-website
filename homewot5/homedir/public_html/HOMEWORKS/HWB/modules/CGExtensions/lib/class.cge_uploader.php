...
class cge_uploader extends cg_fileupload
{
  ...
  protected function preprocess_upload($fileinfo)
  {
    ...
    if( $this->_do_preview && $this->_preview_size > 0 && $this->_delete_orig )
      {
	// I guess we're creating a preview.
	$destdir = dirname($srcname);
	$destname = 'rs_'.basename($srcname);
	$tmpname = $destdir.'/'.$tmpname;

	cge_image::transform_image($src,$dest,$this->_preview_size);
  ...
  public function handle_upload($name,$destfilename = '',$subfield = false)
  {
    ...
    $src = cms_join_path($this->get_dest_filename());
    ...
    if( $this->_do_preview == 1 && $this->_preview_size > 0 && $this->_delete_orig == 0 )
      {
	// looks like we're making a preview
	$bn = basename($this->get_dest_filename());
	$filename = 'preview_'.$bn;
	$dest = cms_join_path($this->get_dest_dir(),$filename);

	// todo: check to see if the input is greater than the preview size.
	cge_image::transform_image($src,$dest,$this->_preview_size);
      }
