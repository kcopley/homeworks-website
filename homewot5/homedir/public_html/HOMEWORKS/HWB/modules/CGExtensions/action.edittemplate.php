...
if( isset($params['errors']) )
  {
    echo $module->ShowErrors($params['errors']);
  }
...
if( $params['mode'] == 'add' )
  {
    ...
    $smarty->assign('templatename',
		    $this->CreateInputText( $id, 'template', "", 40, 200 ));
    $smarty->assign('hidden',
		    $this->CreateInputHidden($id, 'prefix', $params['prefix']).
		    $this->CreateInputHidden($id, 'cg_activetab',
					     $this->_current_tab));
	...
	if( !$contents )
	  {
	    $contents = $module->GetPreference($params['defaulttemplatepref']);
	  }
      }
  }
 else 
...
if( method_exists($module, 'GetEditTemplateMessage') )
  ...
  {
    $smarty->assign('template_info',trim($params['info']));
  }
